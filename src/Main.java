...
public class Main {
    public static void main(String[] args) {
        ...
        biletService.setBilete((ArrayList<Bilet>) BiletSingleton.getInstance().getBilete());
            ...
            try {
                switch (command) {
                    case "1":
                        ...
                        avionService.addAvioane(avion);
                        audit.denumireActiune("Creare avion nou");
                        break;
                    case "2":
                        bilet =  biletManager.creareBilet(in);
                        biletService.addBilet(bilet);
                        audit.denumireActiune("Creare bilet nou");
                        break;
                    case "3":
                        persoana =  persoanaManager.crearePersoana(in);
                        persoanaService.addPersoana(persoana);
                        audit.denumireActiune("Creare persoana noua");
                        break;
                    case "4":
                        zbor = zborManager.creareZbor(in);
                        zborService.addZbor(zbor);
                        audit.denumireActiune("Creare zbor nou");
                        break;
                    case "5":
                        System.out.println(avionService.calculConsumKerosen(avionManager.creareAvion(in)));
